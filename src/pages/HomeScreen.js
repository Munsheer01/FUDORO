...
const SERVICE_SECTIONS = [
  {
    key: "meal-boxes",
    title: "Meal Boxes",
    description: "Curated meal boxes for individuals and families. Fresh, healthy, and delivered to your doorstep. Perfect for daily meals or special occasions.",
  ...
  },
  {
    key: "bulk-orders",
    title: "Bulk Orders",
    description: "Order in bulk for offices, hostels, or events. Hygienic, reliable, and customizable to your needs. Enjoy seamless delivery and great value.",
  ...
  },
  {
    ...
    description: "Professional catering for weddings, parties, and corporate events. Choose from a wide range of cuisines and menu options.",
  ...
  },
  {
    key: "live-counters",
    title: "Live Counters",
    description: "Add excitement to your event with live food counters. Our chefs prepare dishes on-site, ensuring freshness and a memorable experience.",
...
const QuickOrderModal = React.memo(({ isOpen, onClose, onProceed, featuredPlatters = [] }) => {
  ...
  const hyderabadPincodes = useMemo(() => [
    '500001', '500002', '500003', '500004', '500008', '500016', 
    '500032', '500081', '500018', '500020', '500034', '500038'
  ], []);

  const checkDeliveryAvailability = useCallback(async (pincode) => {
    setLoading(true);
    try {
      ...
      setFormData(prev => ({ ...prev, isDeliveryAvailable: isAvailable }));
      setStep(isAvailable ? 3 : 2);
    } catch (error) {
      setErrors({ pincode: 'Unable to check delivery. Please try again.' });
    } finally {
      setLoading(false);
    }
  }, [hyderabadPincodes]);

  const handlePincodeSubmit = useCallback((e) => {
    e.preventDefault();
    setErrors({});
    ...
    if (!/^\d{6}$/.test(formData.pincode)) {
      setErrors({ pincode: 'Pincode should contain only numbers' });
      return;
    }
  ...
  const resetModal = useCallback(() => {
    setStep(1);
    setFormData({ pincode: '', plates: 15, selectedService: null, isDeliveryAvailable: null });
    setErrors({});
    setLoading(false);
  }, []);
  ...
  return (
    <div className={styles.modalOverlay} onClick={handleClose} role="dialog" aria-modal="true" aria-labelledby="modal-title">
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        {step === 1 && (
          <div className={styles.modalStep}>
            <h2 id="modal-title" className={styles.modalTitle}>Quick Order</h2>
            <p className={styles.modalDesc}>Enter your pincode to check delivery availability in Hyderabad</p>
            
            <form className={styles.modalForm} onSubmit={handlePincodeSubmit}>
              <div className={styles.inputGroup}>
                ...
                <input
                  ...
                  inputMode="numeric"
                  pattern="[0-9]{6}"
                  className={`${styles.modalInput} ${errors.pincode ? styles.inputError : ''}`}
                ...
                />
                {errors.pincode && (
                  <span id="pincode-error" className={styles.errorText} role="alert">
                    {errors.pincode}
                  </span>
                )}
              </div>
        ...
        {step === 2 && (
          <div className={styles.modalStep}>
            <div className={styles.notAvailableIcon} aria-hidden="true">😔</div>
            <h2 className={styles.modalTitle}>
              We don't deliver to <strong>{formData.pincode}</strong> yet
            </h2>
            <p className={styles.modalDesc}>
              But we're expanding fast! We'll notify you when we start delivering to your area.
            </p>
        ...
        {step === 3 && (
          <div className={styles.modalStep}>
            <div className={styles.availableIcon} aria-hidden="true">🎉</div>
            <h2 className={styles.modalTitle}>
              Great! We deliver to <strong>{formData.pincode}</strong>
            </h2>
            <p className={styles.modalDesc}>
              You can now proceed with your bulk order. Minimum order: 15 plates.
            </p>
            
            <div className={styles.plateSelector}>
              <label htmlFor="plate-count" className={styles.visuallyHidden}>Number of plates</label>
              <button
                type="button"
              ...
              <div className={styles.plateCount}>
                <span className={styles.plateNumber}>{formData.plates}</span>
    ...
    >
      <div className={styles.imageContainer}>
        <img
          ...
          className={`${styles.serviceImage} ${imageLoaded ? styles.loaded : ''}`}
        ...
        <div className={styles.titleOverlay}>
          ...
          <p className={styles.plateRange}>
            {service.minPlates}-{service.maxPlates} {service.minPlates === 1 ? 'serving' : 'plates'}
          </p>
        </div>
        ...
        <div className={styles.serviceInfo}>
          <div className={styles.plateInfo}>
            <span className={styles.plateInfoText}>
              Perfect for {service.minPlates}-{service.maxPlates} {service.minPlates === 1 ? 'serving' : 'plates'}
            </span>
          </div>
  ...
  return (
    <article className={styles.featuredPlatter}>
      <div className={styles.platterImageContainer}>
        <img
          src={imageError ? '/assets/platter-placeholder.jpg' : platter.imageUrl}
        ...
        {platter.isVeg && (
          <div className={styles.vegBadge}>🟢</div>
        )}
      </div>
      ...
      <div className={styles.platterContent}>
        ...
        <div className={styles.platterMeta}>
          ...
          <span className={styles.serves}>{platter.serves?.display}</span>
        </div>
...
const HomeScreen = () => {
  ...
  // Fetch featured platters with real-time updates
  const fetchFeaturedPlatters = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);

      console.log('Fetching featured platters...');
      
      // Try with simple query first
      const snapshot = await getDocs(collection(db, "Enhanced_Authentic_Platters"));
      ...
      console.log('Featured platters loaded:', activePlatters.length);
      setFeaturedPlatters(activePlatters);

    } catch (err) {
      console.error("Error fetching featured platters:", err);
      setError(`Failed to load featured items${retryCount > 0 ? ` (Attempt ${retryCount + 1})` : ''}`);
      setFeaturedPlatters([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExploreService = useCallback((service) => {
    // For Meal Boxes and Catering Services, go directly to their pages
    if (service.key === "meal-boxes" || service.key === "catering-services") {
      navigate(service.link);
      return;
    }
    // For other services, pass state as before
    navigate(service.link, { 
      state: { 
  ...
  // Handle platter ordering
  const handleOrderPlatter = useCallback((platter) => {
  ...
  // Handle quick order proceed
  const handleQuickOrderProceed = useCallback((formData) => {
    navigate("/bulk-orders", {
      state: {
      ...
      <section className={styles.heroSection}>
        <div className={styles.heroContent}>
          ...
          <p className={styles.heroDesc}>
            Perfect for offices, hostels, events, and celebrations in Hyderabad
          </p>
  ...
  <p className={styles.sectionSubtitle}>Choose from our range of food services for any occasion</p>
    ...
    {SERVICE_SECTIONS.map((service, index) => (
      ...
      />
        ...
        <section className={styles.featuredSection} aria-labelledby="featured-title">
          <h2 id="featured-title" className={styles.sectionTitle}>Featured Platters</h2>
          <p className={styles.sectionSubtitle}>Popular choices from our authentic collection</p>
